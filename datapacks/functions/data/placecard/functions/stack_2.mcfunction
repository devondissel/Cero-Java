...
execute as @s if score @e[tag=dummy,limit=1] compatible matches 1 run tellraw @a ["",{"selector":"@a[tag=turn,limit=1]","color":"aqua"},{"text":" placed down a draw card. The next player in sequence has to draw ","color":"aqua"},{"score":{"name":"@e[tag=dummy]","objective":"hastodraw2"},"color":"aqua"},{"text":" cards or choose to stack another draw card on top.","color":"aqua"}]
