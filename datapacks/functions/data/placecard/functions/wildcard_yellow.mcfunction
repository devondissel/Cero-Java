...
execute if entity @e[team=green,tag=wild] run tellraw @a ["",{"selector":"@a[tag=turn,limit=1]","color":"aqua"},{"text":" colocó una carta comodín y cambió el color a amarillo.","color":"aqua"}]
execute if entity @e[team=green,tag=wild+4] run tellraw @a ["",{"selector":"@a[tag=turn,limit=1]","color":"aqua"},{"text":" colocó una carta comodín y cambió el color a amarillo. El siguiente jugador tiene que robar 4 cartas y pierde un turno.","color":"aqua"}]
