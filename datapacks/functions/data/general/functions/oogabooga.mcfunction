...
execute if entity @a[tag=turn] unless entity @a[scores={cardcount=0}] unless entity @e[tag=dummy,scores={deckshuffle2=1..}] run tellraw @a ["",{"selector":"@e[tag=turn]","color":"gold"},{"selector":"@e[tag=stacking]","color":"gold"},{"text":" is playing.","color":"gold"}]
